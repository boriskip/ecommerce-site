...
export default function EditFlashSaleModal({ isOpen, onClose, onSuccess, flashSale }) {
    ...
    // 2. При открытии модалки заполняем форму данными flashSale
    useEffect(() => {
        if (isOpen && flashSale) {
            setForm({
                product_id: flashSale.product_id,
                price: flashSale.price,
                old_price: flashSale.old_price,
                discount: flashSale.discount,
                rating: flashSale.rating,
                reviews: flashSale.reviews,
                starts_at: flashSale.starts_at,
                ends_at: flashSale.ends_at,
            });
            setDiscount(flashSale.discount);
        }
    }, [isOpen, flashSale]);

    // 3. Обновляем выбранный продукт и old_price
    useEffect(() => {
        if (form.product_id) {
    ...
    // 4. Автоматический пересчет цены при изменении скидки или товара
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        try {
            await axiosPrivate.put(`/api/admin/flash-sales/${flashSale.id}`, form);
            onSuccess && onSuccess();
            onClose();
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
                <h2 className="text-2xl font-bold mb-4">Редактировать Flash Sale</h2>
